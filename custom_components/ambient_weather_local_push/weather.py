...
class AmbientWeatherWeather(AmbientWeatherEntity, WeatherEntity):
    ...
    def update_from_latest_data(self) -> None:
        ...
        is_cloudy = (
            light_level < WEATHER_LIGHT_LEVEL_PARTLYCLOUDY
            and light_level > WEATHER_LIGHT_LEVEL_NIGHT
        )
        is_partlycloudy = light_level >= WEATHER_LIGHT_LEVEL_PARTLYCLOUDY
        is_lightning = (
            now is not None
            and last_lightning is not None
            and now - last_lightning <= timedelta(minutes=10)
        )
